...
def experiment(series, updates,look_back,neurons,n_epoch):
    ...
    train_size = int(dataset.shape[0] -20)
    ...
    # rmse_test = sqrt(mean_squared_error(raw_values[-len(test_scaled):-1], predictions_test[1:]))
    # print('Test RMSE: %.3f' % rmse_test)
    # #report performance using RMSPE
    # rmspe_test = RMSPE(raw_values[-len(test_scaled):-1],predictions_test[1:])
    # print('Test RMSPE: %.3f' % rmspe_test)
    # MAE_test = mean_absolute_error(raw_values[-len(test_scaled):-1], predictions_test[1:])
    # print('Test MAE: %.5f' % MAE_test)
    # MAPE_test = MAPE(raw_values[-len(test_scaled):-1], predictions_test[1:])
    ...
    for i in range(50):
        predictions_test[i]=predictions_test[i][0]
        val[i]=val[i][0]
        
     # report performance using RMSE
    rmse_test = sqrt(mean_squared_error(val, predictions_test))
    print('Test RMSE: %.3f' % rmse_test)
    #report performance using RMSPE
    rmspe_test = RMSPE(val,predictions_test)
    print('Test RMSPE: %.3f' % rmspe_test)
    ...
    ax.set_ylabel('Dinamic DLSTM',fontsize = 16)
    plt.show()
  
    ig, ax = plt.subplots(1)
    ax.plot(val, label='original', color='blue')
    ax.plot(predictions_test, label='predictions', color='red')
    ax.legend(loc='upper right')
    ax.set_xlabel("Time",fontsize = 16)
    ax.set_ylabel('Dinamic DLSTM 200',fontsize = 16)
    plt.show()
    return (predictions_test,val)
      ...
      #load dataset
series = read_csv('prueba10000q.csv',header=0,index_col=0)
look_back= 4
neurons= [ 2 , 5 ]
n_epochs=5
updates= 1
...
#  p,q=experiment(series, updates,look_back,neurons,n_epochs)
