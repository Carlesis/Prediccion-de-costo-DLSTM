...
def data_analysis_univariate(df, parent_folder, series_name):
    ...
    def stat_table(dataframe, table, path_name):
        ...
        Path(f"{path_name}/Dataset Summary Statistics ("+str(df.columns[0])+").txt").write_text(str(stat_table))
        print(stat_table)
    ...
    for col in df.columns:
        plot_acf(df[col], title='Autocorrelation Function for '+str(col),lags=50)
        plt.xlabel('Lags')
        plt.ylabel('ACF')
        plt.savefig(f"{univariate_figures}/Autocorrelation Function ("+str(col)+").png")
        plt.close()
        plot_pacf(df[col], title='Partial Autocorrelation Function for '+str(col), lags=50) #lags=50   
        plt.xlabel('Lags')
        plt.ylabel('PACF')
        plt.savefig(f"{univariate_figures}/Partial Autocorrelation Function ("+str(col)+").png")
        plt.close()
    ## FULL VARIABLE PLOTS ##
    ...
    plt.savefig(f"{univariate_figures}/Diagnostic Plots ("+str(df.columns[0])+").png", format="png")
    plt.close()
    ##CREATE DIFFERENCED DATASET##
    df_diff=df.diff().dropna()
    ##CHECK PLOTS##
    plot_diagnostics(df_diff, str(df.columns[0])+" (Differenced)")
    plt.savefig(f"{univariate_figures}/Diagnostic Plots ("+str(df.columns[0])+" Differenced Series).png", format="png")
    plt.close()
    ...
    Path(f"{univariate_tables}/Augmented Dickey Fuller Test on "+str(df.columns[0])+" Series.txt").write_text(adfuller_test(df, name=df.columns[0]))
    ##AND ON DIFFERENCED SERIES##
    Path(f"{univariate_tables}/Augmented Dickey Fuller Test on "+str(df.columns[0])+" Series (Differenced).txt").write_text(adfuller_test(df_diff, name=str(df.columns[0])+' (Differenced Series)'))
#####################################################################################################################################
...
def arima(df, parent_folder, series_name):
    ...
    Path(f"{univariate_tables}/ARIMA "+str(order)+" Output "+str(df.columns[0])+" Series.txt").write_text(str(arima_model.summary()))
    arima_model.plot_diagnostics(figsize=(8,6))
    plt.savefig(f"{univariate_arima}/ARIMA "+str(order)+" Model Plot Diagnostics "+str(df.columns[0])+".png", format="png")
    plt.close()
    ...
    univariate_baseline_training_times_table.title = 'Univariate Baseline Models Training Times ('+str(df.columns[0])+')'
    ...
    univariate_baseline_table_train.title = 'Univariate In-Sample Baseline Model Performances ('+str(df.columns[0])+')'
    ...
    univariate_baseline_table_test.title = 'Univariate Out-of-Sample Model Performances ('+str(df.columns[0])+')'
    ...
    return arima_error_train, arima_error_test,arima_forecasts_test
#####################################################################################################################################
...
def ffnn_univariate(df, parent_folder, series_name, L, ffnn_nodes1, ffnn_nodes2, ffnn_epochs, ffnn_batch, ffnn_optimizer, T=1):
    ...
    def fill_missing_values(dataframe, method='ffill'):
        ...
        if missing_values==True:
            ...
            print("The dataset does not contain missing values")
    fill_missing_values(df)
    ##FILL REuniING MISSING VALUES WITH ZEROS##
    df=df.fillna(0)
    ##SPLIT TRAINING AND TEST SETS##
    def train_test (dataset, train):
        """Define and operate train-test split on dataset and return the length of the test set as scalar"""
...
def lstm_univariate(df, parent_folder, series_name, L, lstm_nodes1, lstm_nodes2, lstm_epochs, lstm_batch, lstm_optimizer, T=1):
    ...
    return univariate_lstm_training_time, lstm_metrics_train, lstm_metrics_test, lstm_layers, lstm_error_train, lstm_error_test, lstm_test_predict_inv
...
def univariate_tables(df, univariate_ffnn_training_time, univariate_rnn_training_time, univariate_lstm_training_time, univariate_gru_training_time, univariate_cnn_training_time,
    ...
    univariate_training_times_table.title = 'Univariate Neural Networks Training Times ('+str(df.columns[0])+')'
    ...
    univariate_table_train.title = 'Univariate In-Sample Model Performances ('+str(df.columns[0])+')'
    ...
    univariate_table_test.title = 'Univariate Out-of-Sample Model Performances ('+str(df.columns[0])+')'
...
def grid_search_ffnn_univariate(df, parent_folder, series_name, lags, nodes1, nodes2, optimizer, epochs, batch):
    ...
    ##Enviar correo
    with open(f"{univariate_grid_search}/FFNN - Grid Search Parameters.txt", "r+") as f:
        data=f.read()
        crearEmail("luis.zapatier@yachaytech.edu.ec", "1206093021", "luis.zapatier@yachaytech.edu.ec", "Resultado de: ", data)
