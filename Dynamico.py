# -*- coding: utf-8 -*-
"""
Created on Wed Aug  3 12:53:51 2022
...
def experiment(series, updates,look_back,neurons,n_epoch):
	...
	train_size = int(dataset.shape[0] -100)
	...
	ax.plot(raw_values[-100:], label='original', color='blue')
	ax.plot(predictions[-100:], label='predictions', color='red')
	ax.legend(loc='upper right')
	ax.set_xlabel("Time",fontsize = 16)
...
def run():
 	...
 	#load dataset
 	series = read_csv('prueba8.csv',header=0,index_col=0)
 	look_back= 4
 	neurons= [ 2 , 5 ]
 	n_epochs=5
 	updates= 1
